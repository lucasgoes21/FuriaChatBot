...
function App() {
  ...
  const handleSendMessageFromSuggestion = async (suggestion) => {
    if (isLoading) return; // Evita enviar enquanto está carregando
  
    setIsLoading(true); // Define o estado de carregamento como verdadeiro
  
    // Adiciona a mensagem da sugestão ao chat
    const newMessages = [...messages, { type: "person", message: suggestion }];
      ...
      >
        {messages.length === 0 ? (
          ...
          >
            ...
            >
              <button
                id="sugestion1"
                className="w-auto h-auto flex items-center justify-center gap-2.5 p-1.5 bg-[#808080] rounded-[12px] text-white hover:bg-[#6c6c6c] transition"
                onClick={() =>
                  handleSendMessageFromSuggestion("Qual é a lineup da FURIA?")
                }
              >
                Qual é a lineup da FURIA?
              </button>
              <button
                id="sugestion2"
                className="w-auto h-auto flex items-center justify-center gap-2.5 p-1.5 bg-[#808080] rounded-[12px] text-white hover:bg-[#6c6c6c] transition"
                onClick={() =>
                  handleSendMessageFromSuggestion(
                    "Quanto foi o último jogo da FURIA?"
                  )
                }
              >
                Quanto foi o último jogo da FURIA?
                ...
                >
                  <ReactMarkdown
                    components={{
                      ...
                      ol: ({ node, ...props }) => (
                        <ol
                          className="text-white list-decimal ml-5"
                          {...props}
                        />
                      ),
                      li: ({ node, ...props }) => (
        ...
        >
          ...
          <button
            ...
            className={`w-[40px] h-[40px] flex items-center justify-center rounded-full text-white transition ${
              isLoading
                ? "bg-gray-500 cursor-not-allowed"
                : "bg-[#808080] hover:bg-[#6c6c6c]"
            }`}
          >
            {isLoading ? (
